...
namespace signlang::audio_frontend {
  ...
  public:
    ...
    [[nodiscard]] auto output_format() const -> AudioFormat;
    [[nodiscard]] auto output_frame_count() const -> std::uint32_t;
    [[nodiscard]] auto publish_period_ms() const -> std::uint32_t;
    void process(const std::vector<std::int16_t>& input_samples, AudioFrame& output_frame);

  private:
    [[nodiscard]] auto input_frame_count() const -> std::uint32_t;
    [[nodiscard]] auto mix_channel(const std::vector<std::int16_t>& input_samples, std::uint32_t input_frame_index,
