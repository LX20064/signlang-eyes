...
auto clamp_to_sample(std::int32_t value) -> std::int16_t {
  const auto clamped_value = std::clamp(value,
...
auto AudioProcessor::output_format() const -> AudioFormat {
...
auto AudioProcessor::publish_period_ms() const -> std::uint32_t {
  return publish_period_ms_;
}

void AudioProcessor::process(const std::vector<std::int16_t>& input_samples, AudioFrame& output_frame) const {
  ...
  for (std::uint32_t output_frame_index = 0; output_frame_index < output_frames; ++output_frame_index) {
    ...
    for (std::uint16_t output_channel_index = 0; output_channel_index < output_format_.channel_count;
      ...
      const auto output_sample_index =
          (output_frame_index * output_format_.channel_count) + output_channel_index;
      ...
      const auto interpolated_sample =
          ((current_sample
            * static_cast<std::int64_t>(output_format_.sample_rate_hz - interpolation_remainder))
...
auto AudioProcessor::mix_channel(const std::vector<std::int16_t>& input_samples,
  ...
  auto last_input_channel = static_cast<std::uint16_t>(
      ((output_channel_index + 1) * input_format_.channel_count) / output_format_.channel_count);
